...
export default function DeploymentVerification() {
  ...
  // Ensure selectedNamespace is valid after namespaces load
  useEffect(() => {
    if (namespaces && namespaces.length > 0) {
      // Get namespace names (handle both object and string formats)
      const namespaceNames = namespaces.map(ns => typeof ns === 'object' ? ns.name : ns);
      
      // If current selection isn't in the list, update to first available or 'default' if exists
      if (!namespaceNames.includes(selectedNamespace)) {
        const defaultNs = namespaceNames.find(n => n === 'default');
        setSelectedNamespace(defaultNs || namespaceNames[0]);
      }
    }
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        <FormControl size="small" sx={{ minWidth: 200 }}>
          ...
          >
            {namespaces.map((ns) => {
              // Handle both object format {name, status, ...} and string format
              const nsName = typeof ns === 'object' ? ns.name : ns;
              return (
