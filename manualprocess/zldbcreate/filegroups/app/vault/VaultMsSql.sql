...
CREATE SEQUENCE pfuser01.dsu_shName_seq
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
-- OPTIONAL
CREATE SEQUENCE pfuser01.dsu_sequence
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
CREATE TABLE pfuser01.DiskStorageUnit (
    -- IDENTITY
   ...
   CONSTRAINT pk_DSU PRIMARY KEY (dsuId) ON ZL_APP_INDEX,
   CONSTRAINT uk_DSU UNIQUE (dsuName) ON ZL_APP_INDEX,
   CONSTRAINT uk2_DSU UNIQUE (dsuShortName) ON ZL_APP_INDEX 
) ON ZL_APP
GO

CREATE INDEX i1_DSU ON pfuser01.DiskStorageUnit(dsuAppId) ON ZL_APP_INDEX
GO




CREATE SEQUENCE pfuser01.dv_shName_seq
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
-- OPTIONAL
CREATE SEQUENCE pfuser01.dv_sequence
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
CREATE TABLE pfuser01.DiskVolume (
   -- IDENTITY
   ...
   CONSTRAINT pk_DiskVolume     PRIMARY KEY (dvId) ON ZL_APP_INDEX,
--   CONSTRAINT fk_DiskVolume     FOREIGN KEY (dvDsuId) REFERENCES pfuser01.DiskStorageUnit(dsuId) ON DELETE CASCADE,
   CONSTRAINT uk_DiskVolume     UNIQUE (dvName) ON ZL_APP_INDEX,
   CONSTRAINT uk2_DiskVolume     UNIQUE (dvShortName) ON ZL_APP_INDEX 
) ON ZL_APP
GO
...
-- OPTIONAL
CREATE SEQUENCE pfuser01.vi_sequence
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
CREATE TABLE pfuser01.VaultItem(
    -- IDENTITY
    ...
    viSize                      BIGINT NULL,
	viAppFlags                  INT  NULL,
    CONSTRAINT pk_VaultItem     PRIMARY KEY (viID) ON ZL_VAULT_INDEX,
--, 
    CONSTRAINT uk_VaultItem     UNIQUE (viStId) ON ZL_VAULT_INDEX 
--     
)
ON ZL_VAULT
GO
...
CREATE INDEX i1_VaultItem ON pfuser01.VaultItem(viCreateDate) ON ZL_VAULT_INDEX
GO


CREATE TABLE pfuser01.VaultContainerRefCount(
    ...
    CONSTRAINT pk_VaultContRefCnt     PRIMARY KEY (vcrcLocation,vcrcDvId) ON ZL_VAULT_INDEX 
--     
--,
--    CONSTRAINT fk_VaultContRefCnt FOREIGN KEY (vcrcDvId) REFERENCES pfuser01.DiskVolume(dvId) ON DELETE CASCADE
) ON ZL_VAULT
GO
-- STORAGE (INITIAL 25M NEXT 25M MINEXTENTS 1 MAXEXTENTS  30 PCTINCREASE 0) PCTFREE 5

CREATE INDEX i2_VCRC ON pfuser01.VaultContainerRefCount(vcrcDvId,vcrcCreateDate) ON ZL_VAULT_INDEX
GO




CREATE TABLE pfuser01.VaultReplication(
	...
	CONSTRAINT pk_VaultRep PRIMARY KEY (repDsuId,repDvId,repType,repPartition) ON ZL_APP_INDEX
--,
--    CONSTRAINT fk_VaultRepDsuId FOREIGN KEY (repDsuId) REFERENCES pfuser01.DiskStorageUnit(dsuId) ON DELETE CASCADE,
--    CONSTRAINT fk_VaultRepDvId FOREIGN KEY (repDvId) REFERENCES pfuser01.DiskVolume(dvId) ON DELETE CASCADE,
--    CONSTRAINT fk_VaultRepType CHECK (repType IN (0, 1, 2))
) ON ZL_APP
GO
CREATE INDEX i1_vaultRep ON pfuser01.VaultReplication(repDvId) ON ZL_APP_INDEX
GO

   
CREATE TABLE pfuser01.StorageContainerLog (
...
CREATE INDEX i1_SCL ON pfuser01.StorageContainerLog(sclDate) ON ZL_TRANSIENT_INDEX
GO

CREATE TABLE pfuser01.AppStorageMap (
	asmApp		VARCHAR(255) NOT NULL,
	asmModule	VARCHAR(255) NOT NULL,
   	asmSubModule	VARCHAR(255) NOT NULL,
   	asmDsuId		INT NULL,
   	asmUpdate	DATETIME NOT NULL,
	CONSTRAINT pk_appStorMap	 PRIMARY KEY (asmApp,asmModule,asmSubModule) ON ZL_APP_INDEX 
--,
--    CONSTRAINT fk_appStorMap FOREIGN KEY (asmDsuId) REFERENCES pfuser01.DiskStorageUnit(dsuId)
) ON ZL_APP
GO


CREATE TABLE pfuser01.VaultItemStats (
	...
	CONSTRAINT pk_ViStats PRIMARY KEY (viStatDvId,viStatPeriodInfo) ON ZL_STATS_INDEX
) ON ZL_STATS
GO

CREATE INDEX i1_ViStats ON pfuser01.VaultItemStats (viStatPeriodStartDate) ON ZL_STATS_INDEX
GO
CREATE INDEX i2_ViStats ON pfuser01.VaultItemStats (viStatDsuId,viStatPeriodStartDate) ON ZL_STATS_INDEX
GO



-- OPTIONAL
CREATE SEQUENCE pfuser01.ContentCollection_seq
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
CREATE TABLE pfuser01.ContentCollection (
    -- IDENTITY
	...
	CONSTRAINT pk_ContColl PRIMARY KEY (ccId)	ON ZL_APP_INDEX,
	CONSTRAINT uk_ContColl UNIQUE (ccVaultId)	ON ZL_APP_INDEX
) ON ZL_APP
GO


-- OPTIONAL
CREATE SEQUENCE pfuser01.StorageZone_seq
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
CREATE TABLE pfuser01.StorageZone (
 -- IDENTITY
	...
	CONSTRAINT pk_StoreZone PRIMARY KEY (zoneId)	ON ZL_APP_INDEX,
	CONSTRAINT uk_StoreZone UNIQUE (zoneName)	ON ZL_APP_INDEX
) ON ZL_APP
GO



CREATE SEQUENCE pfuser01.cloudStore_seq
INCREMENT BY 1
START WITH 1
NO MAXVALUE
NO CYCLE
...
CREATE TABLE pfuser01.CloudStore (
	...
	CONSTRAINT pk_cloudStore PRIMARY KEY (storeId)	ON ZL_APP_INDEX,
	CONSTRAINT uk_cloudStore UNIQUE (storeName)	ON ZL_APP_INDEX
) ON ZL_APP
GO

CREATE TABLE pfuser01.ExternalWorm (
	...
	CONSTRAINT pk_eworm PRIMARY KEY (ewId)	ON ZL_APP_INDEX
) ON ZL_APP
GO
